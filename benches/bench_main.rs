...
use criterion::*;
use uri_rs::parser;
use uri_rs::parser::parsers::Elms;
...
#[inline]
fn uri_parsers_uri() {
  let _ = parser::parsers::uri_parsers::uri(Elms::new(uri.as_bytes())).unwrap();
}
...
fn uri_hier_part_parsers_hier_part() {
  let _ = parser::parsers::hier_part_parsers::hier_part(Elms::new(hier_part.as_bytes())).unwrap();
}

const query: &'static str = "key1=value1&key2=value2&key1=value2";

fn uri_query_parsers_query() {
  let _ = parser::parsers::query_parsers::query(Elms::new(query.as_bytes())).unwrap();
}
